...
export const ALL_TABS = ["overview", "sales", "ads", "ops", "meta", "winners", "brain", "forecast"];
...
const TAB_DATA_KEYS = {
  ...
  // /team —— 分享给团队的预算推算页。故意不含 meta（Meta 广告后台指标团队用不上）。
  forecast: ["ads", "adsMonths", "adsDaily", "branches", "branchMonths", "branchDaily", "outletSales", "dailySales"],
};
...
export function resolveUser(password) {
  ...
  // 团队分享链接的 key（/team?k=…）。只开 forecast 一个 tab —— filterData 会把
  // 其余数据键清空，所以团队的浏览器里根本收不到 Sales / Ads / Meta 那些数据。
  const teamKey = process.env.TEAM_KEY;
  if (teamKey && password === teamKey) return { name: "Team", tabs: ["forecast"] };
