...
const V_BASE = `https://graph.facebook.com/${V}`;
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// 同步分页：小页 + 跟 paging.next 游标
async function syncPaged(baseUrl, limit) {
  const sep = baseUrl.includes("?") ? "&" : "?";
  let url = /[?&]limit=/.test(baseUrl) ? baseUrl : `${baseUrl}${sep}limit=${limit || 50}`;
  const out = [];
  for (let p = 0; p < 80 && url; p++) {
...
// Meta「异步报表」(submit → poll → fetch)：规避同步单次的「Please reduce the amount of data」
async function asyncReport(baseUrl) {
  const qi = baseUrl.indexOf("?");
  const endpoint = baseUrl.slice(0, qi);
  const params = new URLSearchParams(baseUrl.slice(qi + 1));
  const token = params.get("access_token");
  const sr = await fetch(endpoint, { method: "POST", body: params });
  const sj = await sr.json();
  if (sj.error) throw new Error(sj.error.message || "Meta error");
  const runId = sj.report_run_id;
  if (!runId) return syncPaged(baseUrl, 25);
  for (let i = 0; i < 40; i++) {
    await sleep(1200);
    const pj = await gj(`${V_BASE}/${runId}?fields=async_status&access_token=${encodeURIComponent(token)}`);
    if (pj.async_status === "Job Completed") {
      return syncPaged(`${V_BASE}/${runId}/insights?limit=100&access_token=${encodeURIComponent(token)}`);
    }
    if (/Failed|Skipped/i.test(pj.async_status || "")) throw new Error("Meta 报表 " + pj.async_status);
  }
  throw new Error("Meta 报表生成超时，请把时间范围调短再试");
}

// 拉 insights：先试同步分页(快)；撞到「数据太多」软上限时退回异步报表(稳)。
// baseUrl: https://graph.facebook.com/vXX/<act>/insights?level=...&fields=...&time_range=...&access_token=...
export async function fetchInsightsPaged(baseUrl) {
  try {
    return await syncPaged(baseUrl, 25);
  } catch (e) {
    if (/reduce the amount of data/i.test(e.message || "")) return asyncReport(baseUrl);
    throw e;
  }
}
